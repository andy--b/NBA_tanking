...
winner_data_fn = function(number_of_wins,from_this_place,start_year,end_year) {
	
	# Initialize blank data frame, structured same as aggr_data
	winner_data = data.frame(team = character(0), year = integer(0), 
	...
	# Loop through each year, append to winner_data frame
	for (year in start_year:end_year) {
	
		print(paste('Now adding winners from ', as.character(year)));
		
		# Create data frame for 'year'
		data_for_year = aggr_data[aggr_data[,"year"] == year,];
		
		# Calculate maximum_wins, which is the most wins a team can have on the winner table
		...
		winners = data_for_year[data_for_year[,"mid_wins"] > maximum_wins,];
		rows_to_append = nrow(winners);
		
		# Make sure we have any rows to append for this year
		if (rows_to_append > 0) {
			# Append winners to winner_data
			for (i in 1:nrow(winners)) {
				winner_data[iter,] = winners[i,];
	...
	print("winner data complete");
	
	# return winner_data
	return(winner_data);	
	}
	
winner_data = winner_data_fn(number_of_wins,from_this_place,start_year,end_year)